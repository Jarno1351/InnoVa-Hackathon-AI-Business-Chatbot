...
export default function App() {
  ...
  async function handleSendMessage(rawMessage) {
    ...
    // Prevent execution if input is empty or a request is already running
    if (!message || isChatLoading) return;

    // 1. Instantly clear input and toggle loading to prevent double-submits
    setMessageInput('');
    setIsChatLoading(true);

    // 2. Generate unique ID for user bubble and push to local view state
    const userMessageId = crypto.randomUUID();
    setMessages((prev) => [...prev, { id: userMessageId, sender: 'user', text: message }]);

    try {
      // 🎯 PAYLOAD ALIGNMENT: Pass the raw string under the key 'message' to match Express req.body
      ...
      // 3. Update both chat history state arrays simultaneously
      setMessages((prev) => {
        const next = [...prev, { id: crypto.randomUUID(), sender: 'bot', text: reply }];
        
        // Map structures cleanly to preserve message context logs
        const formattedHistoryMessages = next.map(({ sender, text }) => ({ sender, text }));

        if (!activeChatId) {
          // FIXED: Changed history schema property key to "messages" (plural) to fix state conflicts
          const historyItem = {
            id: crypto.randomUUID(),
          ...
          setChatHistory((history) => 
            history.map((item) => 
              item.id === activeChatId 
                ? { ...item, messages: formattedHistoryMessages } 
                : item
            )
          );
        }
      ...
      // 4. If RAG Engine hit database matches, open the interactive drawer overlay
      ...
      // Fallback UI grace handling in case Express or Flask is completely down
      ...
      // Always toggle loading lock off when completely finished
  ...
  function handleContactSupplier() {
    alert('Contact supplier action simulated. This can later open chat, call, email, or supplier contact details.');
  }
  ...
  return (
    <>
      ...
      <Sidebar
        ...
        onShowChat={showChatView}
        onShowShop={showShopView}
      ...
      <main className="app-shell">
        ...
        {activeView === 'shop' && (
          <ShopView shops={shops} onBrowseShop={(shop) => openRecommendationDrawer([shop])} onOpenSidebar={openMobileSidebar} />
        )}

        {activeView === 'dashboard' && <MerchantDashboard user={user} />}

        <button className="dashboard-floating-button" onClick={showDashboardView} type="button">Dashboard</button>
      </main>
